...
\usepackage{listings}
\usepackage{caption}
\captionsetup[lstlisting]{position=top, skip=10pt, belowskip=0pt}
...
% Parâmetros globais de colocação de floats: dá ao LaTeX mais flexibilidade
% para evitar paragrafos "viúvos" entre figuras/tabelas e a sua secção.
...
Dentro deste paradigma, \textit{Function as a Service} (FaaS) é a forma mais comum de aplicar serverless na prática. Uma função FaaS é um bloco de código simples, sem estado, que é executado sempre que um evento acontece, podendo ser um pedido HTTP, uma mensagem numa fila, um ficheiro escrito num bucket, ou uma tarefa agendada. O modelo é dividido em três propriedades operacionais:
...
\textbf{Ausência de estado.} Como uma função pode ser executada em qualquer instância e desaparecer assim que termina, o estado tem de residir noutro local como uma base de dados, uma cache distribuída ou um sistema de mensagens. Esta restrição é o que torna possível a escalabilidade horizontal sem coordenação explícita \cite{jonas2019,hellerstein2019}.
...
\node[box, fill=orange!20, right=of cli] (gw)   {\textit{Gateway}};
\node[box, fill=green!15,  right=of gw]  (sb)   {\textit{Sandbox}\\(Deno)};
\node[box, fill=purple!15, right=of sb]  (fn)   {Função\\primes};

\draw[arr] (cli) -- node[arrlbl]{POST}    (gw);
\draw[arr] (gw)  -- node[arrlbl]{routing} (sb);
\draw[arr] (sb)  -- node[arrlbl]{invoke}  (fn);
\draw[arr, dashed] (fn.south) to[bend left=35]
      node[arrlbl, below=1pt]{response} (cli.south);

\node[font=\scriptsize\itshape, below=4pt of sb, text=black!55] {\textit{cold} ou \textit{warm}};
\end{tikzpicture}
...
%=================================================================
O mercado FaaS está hoje consolidado em torno de um pequeno conjunto de plataformas com capacidades semelhantes, mas que se distinguem pelo ecossistema em que se inserem e pelo modelo de execução adotado. Apresentam-se de seguida as plataformas consideradas neste trabalho.

\textbf{AWS Lambda.} É o ponto de referência histórico do modelo, lançado em 2014. Suporta múltiplos \textit{runtimes} (Node.js, Python, Java, Go, .NET, Ruby) e integra-se com a totalidade do catálogo da AWS. As funções correm em \textit{micro-VMs}, o que oferece bom isolamento mas \textit{cold starts} relativamente altos para \textit{runtimes} pesados.
...
\textbf{Google Cloud Functions para Firebase \cite{firebasefunctions}.} Pensadas para complementar o ecossistema móvel do Firebase, são particularmente convenientes para reagir a eventos de autenticação, escrita em Firestore ou alterações em \textit{Cloud Storage}. Podem ser escritas em Node.js, Python ou Go.

\textbf{IBM Cloud Code Engine \cite{ibmcodeengine}.} Plataforma compatível com Kubernetes, executa tanto funções como \textit{containers} de longa duração. A flexibilidade do modelo é simultaneamente vantagem e desvantagem: requer um pouco mais de configuração inicial do que as alternativas estritamente FaaS.
...
\begin{lstlisting}[basicstyle=\ttfamily\scriptsize, caption={Função \textit{serverless} \texttt{primes}: crivo de Eratóstenes exposto como \textit{endpoint} HTTP.}, label={lst:primes}]
import "@supabase/functions-js/edge-runtime.d.ts";
import { withSupabase } from "@supabase/server";

// Crivo de Eratostenes: devolve primos <= n.
    ...
    async (req) => {
      ...
      if (typeof number !== "number"
        ...
        return Response.json(
          { error: "Envie inteiro >= 0 em 'number'." },
          { status: 400 },
        );
      }
...
\node[step, fill=blue!12]                  (init)  {init};
\node[step, fill=green!15,  right=of init] (new)   {new};
\node[step, fill=orange!18, right=of new]  (serv)  {serve};
\node[step, fill=yellow!25, right=of serv] (link)  {link};
\node[step, fill=purple!15, right=of link] (cfg)   {config};
\node[step, fill=teal!15,   right=of cfg]  (dep)   {deploy};
...
\end{tikzpicture}
\caption{Etapas de \textit{deployment} de uma \textit{Edge Function} no Supabase. Cada caixa corresponde ao subcomando da CLI \texttt{npx supabase <caixa>} (\textit{e.g.}, \texttt{functions new}, \texttt{functions serve}, \texttt{functions deploy}), entre os quais se intercala o ajuste do \texttt{config.toml}.}
\label{fig:deploy}
\end{figure}
...
%=================================================================
A avaliação experimental incidiu sobre três pilares: latência sequencial, latência em utilização concorrente e \textit{cold start}. O estudo foi conduzido em duas etapas distintas e independentes. Na primeira, a \textbf{geração de dados}, foram efetuados pedidos reais para a função em produção, com os tempos de resposta gravados em ficheiro. Na segunda, a \textbf{análise}, esses ficheiros são lidos por um \textit{script} Python (\texttt{pandas} e \texttt{matplotlib}) que calcula percentis e produz os gráficos apresentados. Esta separação permite repetir a análise sobre os mesmos dados sem refazer a bateria de testes.
...
Como as distribuições são assimétricas, médias isoladas podem enganar — um único pedido lento é suficiente para distorcer o resultado. Usa-se por isso \textit{percentis}:

\begin{itemize}
  \item \textbf{p50} (mediana): representa o caso típico.
  \item \textbf{p95}: apenas 5\% dos pedidos foram mais lentos — o ``pior comum''.
  \item \textbf{p99}: isola o 1\% mais lento, útil para detetar \textit{outliers} como \textit{cold starts} ou \textit{rate limiting}.
\end{itemize}
